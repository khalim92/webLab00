...
<html>
	...
	<body>
		...
		<p> You didn't fill out the form completely. <a href="http://localhost/gradestore.html">Try Again?</a> </p>
		...
		<?php
			...
			$now .= $_POST["name"] .";" .$_POST["ID"] .";" .$_POST["credit_card_number"] .";" .$_POST["credit"] ."\n";
			file_put_contents($filename, $now, FILE_APPEND | LOCK_EX);
			$now = file_get_contents($filename);
			?>
			<?= $now ?>
		<?php 
